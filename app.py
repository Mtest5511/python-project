...
users = pd.DataFrame({
"user_id": [1, 2, 3, 4, 5],
"name": ["Alice", "Bob", "Charlie", "Dane", "Eli"],
...
#print(watch.loc[2, "watch_minutes"])    # output : 45

watch.loc[2, "watch_minutes"] = np.nan
print(watch.loc[2, "watch_minutes"])# output : nan
...
#in sql - SELECT genre, SUM(watch_minutes)
# FROMwatch_history
# GROUPBY genre
...
full_data["genre"]= full_data.groupby("genre")["watch_minutes"].transform("mean")
print(full_data)
